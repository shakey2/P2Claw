...
- **Stable surfaces for optional modules:** `tools/registry`, `ToolDefinition` / `tool-types.ts`, `Frontend` / `ui/core` — **not** `security.ts` or credential plumbing unless the change is intentionally security-scoped (rare).
...
├── core/modules/      # Module framework — §4.7
...
├── modules/          # First-party modules — manifest.json + entry (§2.8, §4.7)
...
**Fixed permission catalog.** Core owns a fixed set of 11 broad permission categories (`time.now`, `log.info`, `memory.read`, `memory.write`, `fs.read_public`, `fs.read_private`, `fs.write_any`, `shell.execute`, `process.spawn`, `net.outbound`, `credentials.read`). Each is labelled `safe` or `high`. Modules cannot declare custom permissions — adding a category requires a Core release. See [src/core/modules/permissions.ts](src/core/modules/permissions.ts).

**Manifest.** Each module ships a `manifest.json` validated in [src/core/modules/manifest.ts](src/core/modules/manifest.ts): reverse-DNS id, semver version, `runtime: "inprocess"` or `"mcp"`, `firstParty: true` (the module's folder name must be a key in `FIRST_PARTY_ALLOWLIST` **and** `manifest.id` must equal the reverse-DNS id that folder is bound to — this folder->id binding prevents a rename-and-swap spoof of first-party identity), permissions must be a subset of the catalog, and each tool's `requires` must be a subset of the module's declared permissions. MCP manifests additionally declare a `mcp` config block (command, args, startup timeout, restart policy, optional env) validated for shell-metacharacter injection and secret-passthrough bans.

**Capability broker.** Modules never import Node builtins directly. The loader hands each module a typed `ModuleContext` ([src/core/modules/broker.ts](src/core/modules/broker.ts), [src/core/modules/types.ts](src/core/modules/types.ts)) whose methods each (1) verify the caller's manifest declared the matching permission, (2) gate high-risk permissions through the existing TOTP approval path ([src/security/approval.ts](src/security/approval.ts)), (3) write a decision to the append-only audit log, and (4) only then dispatch the primitive. The **safe** primitives (`log.info`, `time.now`, `memory.read/write`, `fs.read_public`) are real: `memory.*` is backed by the `module_memory` SQLite table and namespaced to the caller's module id ([src/memory/module-store.ts](src/memory/module-store.ts)); `fs.read_public` is scoped to `data/public/<moduleId>/` with path-containment and a 1 MiB read cap. The **high-risk** primitives `shell.execute` and `process.spawn` dispatch real subprocess execution via [src/core/modules/subprocess.ts](src/core/modules/subprocess.ts) (bounded timeout, output cap, env allowlist); `fs.read_private` and `fs.write_any` dispatch real file I/O enforced by [src/core/modules/fs-policy.ts](src/core/modules/fs-policy.ts) (hard bans on `.env`, source tree, audit log, `p2claw.db`; size caps). `net.outbound` and `credentials.read` remain gate-only stubs — no current module needs raw in-process HTTP or credential access, and real implementations would require URL allowlisting / response-size policy and extreme capability-escalation justification respectively.
...
- [x] Shell command tool (real subprocess execution via `child_process.spawn` with 10s default / 60s max timeout, 64 KiB output caps, env allowlist, TOTP-gated approval, subprocess audit events — see `src/core/modules/subprocess.ts`, `src/core/modules/broker.ts`.)
- [x] File system tool (`file_read` / `file_write` / `file_list` sandboxed to `data/workspace/`; hard bans on `.env`, source tree, audit log, `p2claw.db`; `file_write` is high-risk / TOTP-gated; broker `fs.read_private` / `fs.write_any` also real with policy enforcement — see `src/core/modules/fs-policy.ts`.)
...
| 2026-04-20 | Part H — Module Settings And HTML Contribution Hooks | Phase 1: modules declare typed settings fields in `manifest.json`; Core validates schemas at boot, renders forms, stores values in `module_settings` SQLite table (namespaced by moduleId), and audits writes (`settings_event` records). Phase 2: modules declare `tabs` in manifest and contribute `renderContent()` callbacks returning typed `TabContentDescriptor` data; Core renders tab pages from structured blocks (heading, paragraph, kv-table, status, pre, settings-form) — no module-supplied JS/HTML/CSS in the main UI context. Settings are read/written via `ctx.settings.read/write` on the broker. `sensitive: true` fields are masked in UI; TOTP gate planned. Navigation dynamically populated via `/api/nav`. All routes remain loopback-only. |
